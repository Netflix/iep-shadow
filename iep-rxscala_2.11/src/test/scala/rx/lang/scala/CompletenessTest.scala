...
class CompletenessTest extends JUnitSuite {
  ...
  def defaultMethodCorrespondence: Map[String, String] = {
    val allMethods = getPublicInstanceAndCompanionMethods(typeOf[iep.rx.Observable[_]])
  ...
  def correspondenceChanges = Map(
      ...
      "cast(Class[R])" -> "[RxJava needs this one because `iep.rx.Observable` is invariant. `Observable` in RxScala is covariant and does not need this operator.]",
  ...
  @Test def printJavaInstanceMethods(): Unit = {
    printMethodSet("Instance methods of iep.rx.Observable",
                   typeOf[iep.rx.Observable[_]])
  }

  @Ignore // because spams output
  @Test def printScalaInstanceMethods(): Unit = {
    printMethodSet("Instance methods of iep.rx.lang.scala.Observable",
                   typeOf[iep.rx.lang.scala.Observable[_]])
  }

  @Ignore // because spams output
  @Test def printJavaStaticMethods(): Unit = {
    printMethodSet("Static methods of iep.rx.Observable",
                   typeOf[iep.rx.Observable[_]].typeSymbol.companionSymbol.typeSignature)
  }

  @Ignore // because spams output
  @Test def printScalaCompanionMethods(): Unit = {
    printMethodSet("Companion methods of iep.rx.lang.scala.Observable",
                   typeOf[iep.rx.lang.scala.Observable.type])
  }
  ...
  @Test def checkScalaMethodPresenceVerbose(): Unit = {
    ...
    val actualMethods = getPublicInstanceAndCompanionMethods(typeOf[iep.rx.lang.scala.Observable[_]]).toSet
    var good = 0
    var bad = 0
  ...
  @Test def checkJavaMethodPresence(): Unit = {
    ...
    println(  "---------------------------------------------\n")
    checkMethodPresence(correspondence.keys, typeOf[iep.rx.Observable[_]])
  }

  @Ignore // because we prefer the verbose version
  @Test def checkScalaMethodPresence(): Unit = {
    checkMethodPresence(correspondence.values, typeOf[iep.rx.lang.scala.Observable[_]])
  }
