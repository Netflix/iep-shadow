...
import iep.rx.lang.scala.schedulers.TestScheduler
import iep.rx.lang.scala.subjects.BehaviorSubject
...
class ObservableTests extends JUnitSuite {
  ...
  @Test def testJavaFirstOrDefault() {
    assertEquals(1, iep.rx.Observable.just(1, 2).firstOrDefault(10).toBlocking().single)
    assertEquals(10, iep.rx.Observable.empty().firstOrDefault(10).toBlocking().single)
    val msg = "msg6251"
    var receivedMsg = "none"
    try {
      iep.rx.Observable.error(new Exception(msg)).firstOrDefault(10).toBlocking().single
    } catch {
