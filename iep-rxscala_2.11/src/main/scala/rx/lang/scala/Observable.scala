...
import iep.rx.exceptions.OnErrorNotImplementedException
import iep.rx.functions.FuncN
import iep.rx.lang.scala.observables.ConnectableObservable
 ...
 * Call this method to subscribe an [[iep.rx.lang.scala.Observer]] for receiving 
 ...
 *
 * It returns a reference to the [[iep.rx.lang.scala.Subscription]] interface. This enables Observers to
 * unsubscribe, that is, to stop receiving items and notifications before the Observable stops
 * sending them, which also invokes the Observer's [[iep.rx.lang.scala.Observer.onCompleted onCompleted]] method.
 *
 ...
 *         the [[iep.rx.lang.scala.Scheduler]] on which Observers subscribe to the Observable
 *
 ...
 * It stores a reference to the Observer in a collection object, such as a `List[T]` object.
 *
 * It returns a reference to the [[iep.rx.lang.scala.Subscription]] interface. This enables [[Subscriber]]s to
 * unsubscribe, that is, to stop receiving items and notifications before the Observable stops
 * sending them, which also invokes the Subscriber's [[iep.rx.lang.scala.Observer.onCompleted onCompleted]] method.
 *
 ...
 *         the [[iep.rx.lang.scala.Scheduler]] on which [[Subscriber]]s subscribe to the Observable
 *
 * @define subscribeAllReturn 
 *         a [[iep.rx.lang.scala.Subscription]] reference whose `unsubscribe` method can be called to  stop receiving items
...
trait Observable[+T]
{
  ...
  import iep.rx.functions._
  import iep.rx.lang.scala.observables.BlockingObservable
  import ImplicitFunctionConversions._
  import JavaConversions._

  private [scala] val asJavaObservable: iep.rx.Observable[_ <: T]
  ...
  def subscribe(subscriber: Subscriber[T]): Subscription = {
    ...
    val thisJava = asJavaObservable.asInstanceOf[iep.rx.Observable[T]]
  ...
  def ++[U >: T](that: Observable[U]): Observable[U] = {
    val o1: iep.rx.Observable[_ <: U] = this.asJavaObservable
    val o2: iep.rx.Observable[_ <: U] = that.asJavaObservable
    toScalaObservable(iep.rx.Observable.concat(o1, o2))
  }
   ...
   */
  def +:[U >: T](elem: U): Observable[U] = {
    val thisJava = this.asJavaObservable.asInstanceOf[iep.rx.Observable[U]]
  ...
  def concat[U](implicit evidence: Observable[T] <:< Observable[Observable[U]]): Observable[U] = {
    ...
    val o3: Observable[iep.rx.Observable[_ <: U]] = o2.map(_.asJavaObservable)
    val o4: iep.rx.Observable[_ <: iep.rx.Observable[_ <: U]] = o3.asJavaObservable
    val o5 = iep.rx.Observable.concat[U](o4)
  ...
  def concatMap[R](f: T => Observable[R]): Observable[R] = {
    toScalaObservable[R](asJavaObservable.concatMap[R](new Func1[T, iep.rx.Observable[_ <: R]] {
      def call(t1: T): iep.rx.Observable[_ <: R] = {
   ...
   *
   * A well-behaved Observable does not interleave its invocations of the [[iep.rx.lang.scala.Observer.onNext onNext]], [[iep.rx.lang.scala.Observer.onCompleted onCompleted]], and [[iep.rx.lang.scala.Observer.onError onError]] methods of
   * its [[iep.rx.lang.scala.Observer]]s; it invokes `onCompleted` or `onError` only once; and it never invokes `onNext` after invoking either `onCompleted` or `onError`.
   ...
   *         Observable, and that synchronously notifies its [[iep.rx.lang.scala.Observer]]s
   */
  ...
  def timestamp: Observable[(Long, T)] = {
    toScalaObservable[iep.rx.schedulers.Timestamped[_ <: T]](asJavaObservable.timestamp())
      .map((t: iep.rx.schedulers.Timestamped[_ <: T]) => (t.getTimestampMillis, t.getValue))
  }
   ...
   * 
   * @param scheduler [[iep.rx.lang.scala.Scheduler]] to use as a time source.
  ...
  def timestamp(scheduler: Scheduler): Observable[(Long, T)] = {
    toScalaObservable[iep.rx.schedulers.Timestamped[_ <: T]](asJavaObservable.timestamp(scheduler))
      .map((t: iep.rx.schedulers.Timestamped[_ <: T]) => (t.getTimestampMillis, t.getValue))
  }
  ...
  def zipWith[U, R](that: Observable[U])(selector: (T, U) => R): Observable[R] = {
    toScalaObservable[R](iep.rx.Observable.zip[T, U, R](this.asJavaObservable, that.asJavaObservable, selector))
  }
   ...
   *
   * @param openings
   *            The [[iep.rx.lang.scala.Observable]] which, when it produces an object, will cause
   *            another buffer to be created.
   * @param closings
   *            The function which is used to produce an [[iep.rx.lang.scala.Observable]] for every buffer created.
   *            When this [[iep.rx.lang.scala.Observable]] produces an object, the associated buffer
   *            is emitted.
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces buffers which are created and emitted when the specified [[iep.rx.lang.scala.Observable]]s publish certain objects.
   */
  def slidingBuffer[Opening](openings: Observable[Opening])(closings: Opening => Observable[Any]): Observable[Seq[T]] = {
    val opening: iep.rx.Observable[_ <: Opening] = openings.asJavaObservable
    val closing: Func1[_ >: Opening, _ <: iep.rx.Observable[_ <: Any]] = (o: Opening) => closings(o).asJavaObservable
    val jObs: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer[Opening, Any](opening, closing)
    Observable.jObsOfListToScObsOfSeq(jObs.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
   ...
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces connected non-overlapping buffers containing at most
   *         `count` produced values.
   */
  def tumblingBuffer(count: Int): Observable[Seq[T]] = {
    val oJava: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer(count)
    Observable.jObsOfListToScObsOfSeq(oJava.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
   ...
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces buffers every `skip` values containing at most
   *         `count` produced values.
   */
  def slidingBuffer(count: Int, skip: Int): Observable[Seq[T]] = {
    val oJava: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer(count, skip)
    Observable.jObsOfListToScObsOfSeq(oJava.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
   ...
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces connected non-overlapping buffers with a fixed duration.
   */
  def tumblingBuffer(timespan: Duration): Observable[Seq[T]] = {
    val oJava: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer(timespan.length, timespan.unit)
    Observable.jObsOfListToScObsOfSeq(oJava.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
   ...
   *            replaced with a new buffer.
   * @param scheduler
   *            The [[iep.rx.lang.scala.Scheduler]] to use when determining the end and start of a buffer.
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces connected non-overlapping buffers with a fixed duration.
   */
  def tumblingBuffer(timespan: Duration, scheduler: Scheduler): Observable[Seq[T]] = {
    val oJava: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer(timespan.length, timespan.unit, scheduler)
    Observable.jObsOfListToScObsOfSeq(oJava.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
   ...
   *            The maximum size of each buffer before it should be emitted.
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces connected non-overlapping buffers which are emitted after
  ...
  def tumblingBuffer(timespan: Duration, count: Int): Observable[Seq[T]] = {
    val oJava: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer(timespan.length, timespan.unit, count)
    Observable.jObsOfListToScObsOfSeq(oJava.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
  ...
  def tumblingBuffer(timespan: Duration, count: Int, scheduler: Scheduler): Observable[Seq[T]] = {
    val oJava: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer(timespan.length, timespan.unit, count, scheduler)
    Observable.jObsOfListToScObsOfSeq(oJava.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
   ...
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces new buffers periodically, and these are emitted after
  ...
  def slidingBuffer(timespan: Duration, timeshift: Duration): Observable[Seq[T]] = {
    ...
    val oJava: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer(span, shift, unit)
    Observable.jObsOfListToScObsOfSeq(oJava.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
  ...
  def slidingBuffer(timespan: Duration, timeshift: Duration, scheduler: Scheduler): Observable[Seq[T]] = {
    ...
    val shift: Long = timespan.toNanos
    val unit: TimeUnit = duration.NANOSECONDS
    val oJava: iep.rx.Observable[_ <: java.util.List[_]] = asJavaObservable.buffer(span, shift, unit, scheduler)
    Observable.jObsOfListToScObsOfSeq(oJava.asInstanceOf[iep.rx.Observable[_ <: java.util.List[T]]])
  }
  ...
  def tumblingBuffer(boundary: => Observable[Any]): Observable[Seq[T]] = {
    val f = new Func0[iep.rx.Observable[_ <: Any]]() {
      override def call(): iep.rx.Observable[_ <: Any] = boundary.asJavaObservable
    }
  ...
  def tumblingBuffer(boundary: Observable[Any], initialCapacity: Int): Observable[Seq[T]] = {
    val thisJava = this.asJavaObservable.asInstanceOf[iep.rx.Observable[T]]
  ...
  def tumbling(boundary: => Observable[Any]): Observable[Observable[T]] = {
    val func = new Func0[iep.rx.Observable[_ <: Any]]() {
      override def call(): iep.rx.Observable[_ <: Any] = boundary.asJavaObservable
    }
    val jo: iep.rx.Observable[_ <: iep.rx.Observable[_ <: T]] = asJavaObservable.window[Any](func)
   ...
   * @param closings
   *            The function which is used to produce an [[iep.rx.lang.scala.Observable]] for every window created.
   *            When this [[iep.rx.lang.scala.Observable]] produces an object, the associated window
   *            is emitted.
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces windows which are created and emitted when the specified [[iep.rx.lang.scala.Observable]]s publish certain objects.
   */
   ...
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces connected non-overlapping windows containing at most
   *         `count` produced values.
   */
   ...
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces windows every `skip` values containing at most
   *         `count` produced values.
   */
   ...
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces connected non-overlapping windows with a fixed duration.
   */
   ...
   *            replaced with a new window.
   * @param scheduler
   *            The [[iep.rx.lang.scala.Scheduler]] to use when determining the end and start of a window.
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces connected non-overlapping windows with a fixed duration.
   */
   ...
   *            The maximum size of each window before it should be emitted.
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces connected non-overlapping windows which are emitted after
   ...
   * @return
   *         An [[iep.rx.lang.scala.Observable]] which produces new windows periodically, and these are emitted after
   *         a fixed timespan has elapsed.
   */
  ...
  /**
   * Registers an function to be called when this Observable invokes [[iep.rx.lang.scala.Observer.onCompleted onCompleted]] or [[iep.rx.lang.scala.Observer.onError onError]].
   *
  ...
  def flatMap[R](f: T => Observable[R]): Observable[R] = {
    toScalaObservable[R](asJavaObservable.flatMap[R](new Func1[T, iep.rx.Observable[_ <: R]]{
      def call(t1: T): iep.rx.Observable[_ <: R] = { f(t1).asJavaObservable }
    }))
  }
  ...
  def flatMap[R](onNext: T => Observable[R], onError: Throwable => Observable[R], onCompleted: () => Observable[R]): Observable[R] = {
    val jOnNext = new Func1[T, iep.rx.Observable[_ <: R]] {
      override def call(t: T): iep.rx.Observable[_ <: R] = onNext(t).asJavaObservable
    }
    val jOnError = new Func1[Throwable, iep.rx.Observable[_ <: R]] {
      override def call(e: Throwable): iep.rx.Observable[_ <: R] = onError(e).asJavaObservable
    }
    val jOnCompleted = new Func0[iep.rx.Observable[_ <: R]] {
      override def call(): iep.rx.Observable[_ <: R] = onCompleted().asJavaObservable
    }
  ...
  def flatMapWith[U, R](collectionSelector: T => Observable[U])(resultSelector: (T, U) => R): Observable[R] = {
    val jCollectionSelector = new Func1[T, iep.rx.Observable[_ <: U]] {
      override def call(t: T): iep.rx.Observable[_ <: U] = collectionSelector(t).asJavaObservable
    }
  ...
  /**
   * Turns all of the notifications from a source Observable into [[iep.rx.lang.scala.Observer.onNext onNext]] emissions,
   * and marks them with their original notification types within [[iep.rx.lang.scala.Notification]] objects.
   *
  ...
  def materialize: Observable[Notification[T]] = {
    toScalaObservable[iep.rx.Notification[_ <: T]](asJavaObservable.materialize()).map(Notification(_))
  }

  /**
   * Asynchronously subscribes and unsubscribes Observers on the specified [[iep.rx.lang.scala.Scheduler]].
   *
   ...
   *
   * @param scheduler
   *            the [[iep.rx.lang.scala.Scheduler]] to perform subscription and unsubscription actions on
   * @return the source Observable modified so that its subscriptions and unsubscriptions happen
   *         on the specified [[iep.rx.lang.scala.Scheduler]]
   */
  ...
  /**
   * Asynchronously notify [[iep.rx.lang.scala.Observer]]s on the specified [[iep.rx.lang.scala.Scheduler]].
   *
   * <img width="640" height="308" src="https://raw.githubusercontent.com/wiki/ReactiveX/RxJava/images/rx-operators/observeOn.png" alt="" />
   *
   * @param scheduler
   *            the [[iep.rx.lang.scala.Scheduler]] to notify [[iep.rx.lang.scala.Observer]]s on
   * @return the source Observable modified so that its [[iep.rx.lang.scala.Observer]]s are notified on the
   *         specified [[iep.rx.lang.scala.Scheduler]]
   */
  ...
  /**
   * Returns an Observable that reverses the effect of [[iep.rx.lang.scala.Observable.materialize]] by
   * transforming the [[iep.rx.lang.scala.Notification]] objects emitted by the source Observable into the items
   ...
   *
   * @return an Observable that emits the items and notifications embedded in the [[iep.rx.lang.scala.Notification]] objects emitted by the source Observable
   *
  ...
  def dematerialize[U](implicit evidence: Observable[T] <:< Observable[Notification[U]]): Observable[U] = {
    ...
    val o2: Observable[iep.rx.Notification[_ <: U]] = o1.map(_.asJavaNotification)
    val o3 = o2.asJavaObservable.dematerialize[U]()
    toScalaObservable[U](o3)
  }

  /**
   * Instruct an Observable to pass control to another Observable rather than invoking [[iep.rx.lang.scala.Observer.onError onError]] if it encounters an error.
   *
   ...
   * expected item to its [[iep.rx.lang.scala.Observer]], the Observable invokes its Observer's
   ...
   * [[iep.rx.lang.scala.Observer.onNext onNext]] method if it is able to do so. In such a case, because no
  ...
  def onErrorResumeNext[U >: T](resumeFunction: Throwable => Observable[U]): Observable[U] = {
    val f: Func1[Throwable, iep.rx.Observable[_ <: U]] = (t: Throwable) => resumeFunction(t).asJavaObservable
    val f2 = f.asInstanceOf[Func1[Throwable, iep.rx.Observable[Nothing]]]
    toScalaObservable[U](asJavaObservable.onErrorResumeNext(f2))
  }

  /**
   * Instruct an Observable to pass control to another Observable rather than invoking [[iep.rx.lang.scala.Observer.onError onError]] if it encounters an error of type `java.lang.Exception`.
   *
   ...
   * control to `resumeSequence` which will invoke the Observer's [[iep.rx.lang.scala.Observer.onNext onNext]]
  ...
  def onExceptionResumeNext[U >: T](resumeSequence: Observable[U]): Observable[U] = {
    val rSeq1: iep.rx.Observable[_ <: U] = resumeSequence.asJavaObservable
    val rSeq2: iep.rx.Observable[Nothing] = rSeq1.asInstanceOf[iep.rx.Observable[Nothing]]
   ...
   * invoking [[iep.rx.lang.scala.Observer.onError onError]] if it encounters an error.
   *
   ...
   * `resumeFunction` to the Observer's [[iep.rx.lang.scala.Observer.onNext onNext]] method.
   *
   * You can use this to prevent errors from propagating or to supply fallback data should errors
   * be encountered.
   *
  ...
  def reduce[U >: T](accumulator: (U, U) => U): Observable[U] = {
    ...
    toScalaObservable[U](asJavaObservable.asInstanceOf[iep.rx.Observable[T]].reduce(func2))
  }

  /**
   * Returns a [[iep.rx.lang.scala.observables.ConnectableObservable]] that shares a single subscription to the underlying
   * Observable that will replay all of its items and notifications to any future [[iep.rx.lang.scala.Observer]].
   *
   * <img width="640" height="515" src="https://raw.githubusercontent.com/wiki/ReactiveX/RxJava/images/rx-operators/replay.png" alt="" />
   *
   * @return a [[iep.rx.lang.scala.observables.ConnectableObservable]] such that when the `connect` function
   *         is called, the [[iep.rx.lang.scala.observables.ConnectableObservable]] starts to emit items to its [[iep.rx.lang.scala.Observer]]s
   */
  ...
  def replay[R](selector: Observable[T] => Observable[R]): Observable[R] = {
    val thisJava = this.asJavaObservable.asInstanceOf[iep.rx.Observable[T]]
    val fJava: Func1[iep.rx.Observable[T], iep.rx.Observable[R]] =
      (jo: iep.rx.Observable[T]) => selector(toScalaObservable[T](jo)).asJavaObservable.asInstanceOf[iep.rx.Observable[R]]
   ...
   * subscribe/unsubscribe behavior of all the [[iep.rx.lang.scala.Observer]]s.
   *
   ...
   * Observable rather than returning a [[iep.rx.lang.scala.observables.ConnectableObservable ConnectableObservable]] for which you must call
  ...
  /**
   * Returns a [[iep.rx.lang.scala.observables.ConnectableObservable]], which waits until the `connect` function is called
   * before it begins emitting items from `this` [[iep.rx.lang.scala.Observable]] to those [[iep.rx.lang.scala.Observer]]s that
   ...
   *
   * @return an [[iep.rx.lang.scala.observables.ConnectableObservable]].
   */
   ...
   * @param duration the sampling rate
   * @param scheduler
   *            the [[iep.rx.lang.scala.Scheduler]] to use when sampling
   * @return an Observable that emits the results of sampling the items emitted by the source
   *         Observable at the specified time interval
   */
   ...
   * @param accumulator
   *            an accumulator function to be invoked on each item emitted by the source
   *            Observable, whose result will be emitted to [[iep.rx.lang.scala.Observer]]s via
   *            [[iep.rx.lang.scala.Observer.onNext onNext]] and used in the next accumulator call.
  ...
  def scan[U >: T](accumulator: (U, U) => U): Observable[U] = {
    val func: Func2[_ >: U, _ >: U, _ <: U] = accumulator
    val func2 = func.asInstanceOf[Func2[T, T, T]]
    toScalaObservable[U](asJavaObservable.asInstanceOf[iep.rx.Observable[T]].scan(func2))
  }
   ...
   *
   * This method returns an Observable that will invoke a subscribing [[iep.rx.lang.scala.Observer]]'s 
   * [[iep.rx.lang.scala.Observer.onNext onNext]] function a maximum of `num` times before invoking
   * [[iep.rx.lang.scala.Observer.onCompleted onCompleted]].
   *
   * @param n
   ...
   *
   * Normally, an Observable that returns multiple items will do so by invoking its [[iep.rx.lang.scala.Observer]]'s 
   * [[iep.rx.lang.scala.Observer.onNext onNext]] method for each such item. You can change
  ...
  def groupBy[K](f: T => K): Observable[(K, Observable[T])] = {
    val o1 = asJavaObservable.groupBy[K](f) : iep.rx.Observable[_ <: iep.rx.observables.GroupedObservable[K, _ <: T]]
    val func = (o: iep.rx.observables.GroupedObservable[K, _ <: T]) => (o.getKey, toScalaObservable[T](o))
  ...
  def groupBy[K, V](keySelector: T => K, valueSelector: T => V): Observable[(K, Observable[V])] = {
    val jo: iep.rx.Observable[iep.rx.observables.GroupedObservable[K, V]] = asJavaObservable.groupBy[K, V](keySelector, valueSelector)
    toScalaObservable[iep.rx.observables.GroupedObservable[K, V]](jo).map {
      go: iep.rx.observables.GroupedObservable[K, V] => (go.getKey, toScalaObservable[V](go))
    }
  }
  ...
  def join[S, R] (other: Observable[S])(leftDurationSelector:  T => Observable[Any], rightDurationSelector: S => Observable[Any], resultSelector: (T, S) => R): Observable[R] = {
    val outer : iep.rx.Observable[_ <: T] = this.asJavaObservable
    val inner : iep.rx.Observable[_ <: S] = other.asJavaObservable
    val left:  Func1[_ >: T, _<: iep.rx.Observable[_ <: Any]] =   (t: T) => leftDurationSelector(t).asJavaObservable
    val right: Func1[_ >: S, _<: iep.rx.Observable[_ <: Any]] =  (s: S) => rightDurationSelector(s).asJavaObservable
    val f: Func2[_>: T, _ >: S, _ <: R] = resultSelector

    toScalaObservable[R](
      outer.asInstanceOf[iep.rx.Observable[T]].join[S, Any, Any, R](
        inner.asInstanceOf[iep.rx.Observable[S]],
        left. asInstanceOf[Func1[T, iep.rx.Observable[Any]]],
        right.asInstanceOf[Func1[S, iep.rx.Observable[Any]]],
  ...
  def groupJoin[S, R](other: Observable[S])(leftDuration: T => Observable[Any], rightDuration: S => Observable[Any], resultSelector: (T, Observable[S]) => R): Observable[R] = {
    val outer: iep.rx.Observable[_ <: T] = this.asJavaObservable
    val inner: iep.rx.Observable[_ <: S] = other.asJavaObservable
    val left: Func1[_ >: T, _ <: iep.rx.Observable[_ <: Any]] = (t: T) => leftDuration(t).asJavaObservable
    val right: Func1[_ >: S, _ <: iep.rx.Observable[_ <: Any]] = (s: S) => rightDuration(s).asJavaObservable
    val f: Func2[_ >: T, _ >: iep.rx.Observable[S], _ <: R] = (t: T, o: iep.rx.Observable[S]) => resultSelector(t, toScalaObservable[S](o))
    toScalaObservable[R](
      outer.asInstanceOf[iep.rx.Observable[T]].groupJoin[S, Any, Any, R](
        inner.asInstanceOf[iep.rx.Observable[S]],
        left.asInstanceOf[Func1[T, iep.rx.Observable[Any]]],
        right.asInstanceOf[Func1[S, iep.rx.Observable[Any]]],
        f)
  ...
  def switchMap[R](f: T => Observable[R]): Observable[R] = {
    toScalaObservable[R](asJavaObservable.switchMap[R](new Func1[T, iep.rx.Observable[_ <: R]] {
      def call(t: T): iep.rx.Observable[_ <: R] = f(t).asJavaObservable
    }))
  }
  ...
  def switch[U](implicit evidence: Observable[T] <:< Observable[Observable[U]]): Observable[U] = {
    ...
    val o3: Observable[iep.rx.Observable[_ <: U]] = o2.map(_.asJavaObservable)
    val o4: iep.rx.Observable[_ <: iep.rx.Observable[_ <: U]] = o3.asJavaObservable
    val o5 = iep.rx.Observable.switchOnNext[U](o4)
    toScalaObservable[U](o5)
  }
  ...
  def merge[U >: T](that: Observable[U]): Observable[U] = {
    val thisJava: iep.rx.Observable[_ <: U] = this.asJavaObservable
    val thatJava: iep.rx.Observable[_ <: U] = that.asJavaObservable
    toScalaObservable[U](iep.rx.Observable.merge(thisJava, thatJava))
  }

  /**
   * This behaves like [[iep.rx.lang.scala.Observable.merge]] except that if any of the merged Observables
   * notify of an error via [[iep.rx.lang.scala.Observer.onError onError]], `mergeDelayError` will
  ...
  def mergeDelayError[U >: T](that: Observable[U]): Observable[U] = {
    toScalaObservable[U](iep.rx.Observable.mergeDelayError[U](this.asJavaObservable, that.asJavaObservable))
  }
  ...
  def flatten[U](implicit evidence: Observable[T] <:< Observable[Observable[U]]): Observable[U] = {
    ...
    val o3: Observable[iep.rx.Observable[_ <: U]] = o2.map(_.asJavaObservable)
    val o4: iep.rx.Observable[_ <: iep.rx.Observable[_ <: U]] = o3.asJavaObservable
    val o5 = iep.rx.Observable.merge[U](o4)
    toScalaObservable[U](o5)
  }
  ...
  def flatten[U](maxConcurrent: Int)(implicit evidence: Observable[T] <:< Observable[Observable[U]]): Observable[U] = {
    ...
    val o3: Observable[iep.rx.Observable[_ <: U]] = o2.map(_.asJavaObservable)
    val o4: iep.rx.Observable[_ <: iep.rx.Observable[_ <: U]] = o3.asJavaObservable
    val o5 = iep.rx.Observable.merge[U](o4, maxConcurrent)
    toScalaObservable[U](o5)
  }

  /**
   * This behaves like `flatten` except that if any of the merged Observables
   * notify of an error via [[iep.rx.lang.scala.Observer.onError onError]], this method will
   * refrain from propagating that error notification until all of the merged Observables have
   * finished emitting items.
   *
   * <img width="640" height="380" src="https://raw.githubusercontent.com/wiki/ReactiveX/RxJava/images/rx-operators/mergeDelayError.png" alt="" />
  ...
  def flattenDelayError[U](implicit evidence: Observable[T] <:< Observable[Observable[U]]): Observable[U] = {
    ...
    val o3: Observable[iep.rx.Observable[_ <: U]] = o2.map(_.asJavaObservable)
    val o4: iep.rx.Observable[_ <: iep.rx.Observable[_ <: U]] = o3.asJavaObservable
    val o5 = iep.rx.Observable.mergeDelayError[U](o4)
    toScalaObservable[U](o5)
  }
  ...
  def combineLatest[U](that: Observable[U]): Observable[(T, U)] = {
    ...
    toScalaObservable[(T, U)](iep.rx.Observable.combineLatest[T, U, (T, U)](this.asJavaObservable, that.asJavaObservable, f))
  }
  ...
  def combineLatestWith[U, R](that: Observable[U])(selector: (T, U) => R): Observable[R] = {
    toScalaObservable[R](iep.rx.Observable.combineLatest[T, U, R](this.asJavaObservable, that.asJavaObservable, selector))
  }
   ...
   *
   * @param timeout
   *            The time each value has to be 'the most recent' of the [[iep.rx.lang.scala.Observable]] to ensure that it's not dropped.
   *
   * @return An [[iep.rx.lang.scala.Observable]] which filters out values which are too quickly followed up with newer values.
  ...
  def debounce(debounceSelector: T => Observable[Any]): Observable[T] = {
    val fJava = new iep.rx.functions.Func1[T, iep.rx.Observable[Any]] {
      override def call(t: T) = debounceSelector(t).asJavaObservable.asInstanceOf[iep.rx.Observable[Any]]
    }
   ...
   *            The time each value has to be 'the most recent' of the [[iep.rx.lang.scala.Observable]] to ensure that it's not dropped.
   * @param scheduler
   *            The [[iep.rx.lang.scala.Scheduler]] to use internally to manage the timers which handle timeout for each event.
  ...
  def timeout[U >: T](timeout: Duration, other: Observable[U]): Observable[U] = {
    val otherJava: iep.rx.Observable[_ <: U] = other.asJavaObservable
    val thisJava = this.asJavaObservable.asInstanceOf[iep.rx.Observable[U]]
  ...
  def timeout(timeoutSelector: T => Observable[Any]): Observable[T] = {
    toScalaObservable[T](asJavaObservable.timeout({ t: T => timeoutSelector(t).asJavaObservable.asInstanceOf[iep.rx.Observable[Any]] }))
  }
  ...
  def timeout[U >: T](timeoutSelector: T => Observable[Any], other: Observable[U]): Observable[U] = {
    ...
    toScalaObservable[U](thisJava.timeout(
      { t: U => timeoutSelector(t.asInstanceOf[T]).asJavaObservable.asInstanceOf[iep.rx.Observable[Any]] },
  ...
  def timeout(firstTimeoutSelector: () => Observable[Any], timeoutSelector: T => Observable[Any]): Observable[T] = {
    toScalaObservable[T](asJavaObservable.timeout(
      { firstTimeoutSelector().asJavaObservable.asInstanceOf[iep.rx.Observable[Any]] },
      { t: T => timeoutSelector(t).asJavaObservable.asInstanceOf[iep.rx.Observable[Any]] }))
  }
  ...
  def singleOption: Observable[Option[T]] = {
    val jObservableOption = map(Some(_)).asJavaObservable.asInstanceOf[iep.rx.Observable[Option[T]]]
   ...
   *
   * If [[iep.rx.lang.scala.Observer.onError]] is invoked the source Observable will be re-subscribed to as many times as defined by retryCount.
   *
   * Any [[iep.rx.lang.scala.Observer.onNext]] calls received on each attempt will be emitted and concatenated together.
   *
   ...
   * <img width="640" height="315" src="https://raw.githubusercontent.com/wiki/ReactiveX/RxJava/images/rx-operators/retry.png" alt="" />
   *
   * If [[iep.rx.lang.scala.Observer.onError]] is invoked the source Observable will be re-subscribed to.
   *
   * Any [[iep.rx.lang.scala.Observer.onNext]] calls received on each attempt will be emitted and concatenated together.
   *
   * For example, if an Observable fails on first time but emits [1, 2] then succeeds the second time and
  ...
  def retryWhen(notificationHandler: Observable[Throwable] => Observable[Any]): Observable[T] = {
    val f: Func1[_ >: iep.rx.Observable[_ <: Throwable], _ <: iep.rx.Observable[_ <: Any]] =
      (jOt: iep.rx.Observable[_ <: Throwable]) => {
  ...
  def repeatWhen(notificationHandler: Observable[Unit] => Observable[Any], scheduler: Scheduler): Observable[T] = {
    val f: Func1[_ >: iep.rx.Observable[_ <: Void], _ <: iep.rx.Observable[_ <: Any]] =
      (jOv: iep.rx.Observable[_ <: Void]) => {
  ...
  /**
   * Converts an Observable into a [[iep.rx.lang.scala.observables.BlockingObservable BlockingObservable]] (an Observable with blocking
   * operators).
   *
   * @return a [[iep.rx.lang.scala.observables.BlockingObservable BlockingObservable]] version of this Observable
  ...
  def amb[U >: T](that: Observable[U]): Observable[U] = {
    val thisJava: iep.rx.Observable[_ <: U] = this.asJavaObservable
    val thatJava: iep.rx.Observable[_ <: U] = that.asJavaObservable
    toScalaObservable[U](iep.rx.Observable.amb(thisJava, thatJava))
  }
  ...
  def delay(itemDelay: T => Observable[Any]): Observable[T] = {
    val itemDelayJava = new Func1[T, iep.rx.Observable[Any]] {
      override def call(t: T): iep.rx.Observable[Any] =
        itemDelay(t).asJavaObservable.asInstanceOf[iep.rx.Observable[Any]]
    }
  ...
  def delay(subscriptionDelay: () => Observable[Any], itemDelay: T => Observable[Any]): Observable[T] = {
    val subscriptionDelayJava = new Func0[iep.rx.Observable[Any]] {
      override def call(): iep.rx.Observable[Any] =
        subscriptionDelay().asJavaObservable.asInstanceOf[iep.rx.Observable[Any]]
    }
    val itemDelayJava = new Func1[T, iep.rx.Observable[Any]] {
      override def call(t: T): iep.rx.Observable[Any] =
        itemDelay(t).asJavaObservable.asInstanceOf[iep.rx.Observable[Any]]
  ...
  def elementAtOrDefault[U >: T](index: Int, default: U): Observable[U] = {
    val thisJava = asJavaObservable.asInstanceOf[iep.rx.Observable[U]]
  ...
  def sequenceEqualWith[U >: T](that: Observable[U])(equality: (U, U) => Boolean): Observable[Boolean] = {
    ...
    toScalaObservable[java.lang.Boolean](iep.rx.Observable.sequenceEqual[U](thisJava, thatJava, equalityJava)).map(_.booleanValue)
  }
...
object Observable {
  ...
  import ImplicitFunctionConversions._
  import JavaConversions._
  import iep.rx.lang.scala.subjects.AsyncSubject

  private[scala]
  def jObsOfListToScObsOfSeq[T](jObs: iep.rx.Observable[_ <: java.util.List[T]]): Observable[Seq[T]] = {
  ...
  private[scala]
  def jObsOfJObsToScObsOfScObs[T](jObs: iep.rx.Observable[_ <: iep.rx.Observable[_ <: T]]): Observable[Observable[T]] = {
    val oScala1: Observable[iep.rx.Observable[_ <: T]] = new Observable[iep.rx.Observable[_ <: T]]{ val asJavaObservable = jObs }
    oScala1.map((oJava: iep.rx.Observable[_ <: T]) => oJava)
  }

  /**
   * Creates an Observable that will execute the given function when an [[iep.rx.lang.scala.Observer]] subscribes to it.
   *
   ...
   * should invoke the Observer's [[iep.rx.lang.scala.Observer.onNext onNext]], [[iep.rx.lang.scala.Observer.onError onError]], and [[iep.rx.lang.scala.Observer.onCompleted onCompleted]] methods
   * appropriately.
   *
   ...
   *            as appropriate, and returns a [[iep.rx.lang.scala.Subscription]] to allow the Observer to
   *            canceling the subscription.
   * @return
   *         an Observable that, when an [[iep.rx.lang.scala.Observer]] subscribes to it, will execute the given function.
   */
  ...
  */
  ...
  def apply[T](f: Subscriber[T] => Unit): Observable[T] = {
    toScalaObservable(iep.rx.Observable.create(f))
  }
  
  /**
   * Returns an Observable that invokes an [[iep.rx.lang.scala.Observer]]'s [[iep.rx.lang.scala.Observer.onError onError]]
   ...
   * @return an Observable that invokes the [[iep.rx.lang.scala.Observer]]'s [[iep.rx.lang.scala.Observer.onError onError]]
   *         method when the Observer subscribes to it
   */
  def error[T](exception: Throwable): Observable[T] = {
    toScalaObservable[T](iep.rx.Observable.error(exception))
  }

  /**
   * Returns an Observable that emits no data to the [[iep.rx.lang.scala.Observer]] and
   * immediately invokes its [[iep.rx.lang.scala.Observer#onCompleted onCompleted]] method
   ...
   *
   * @return an Observable that returns no data to the [[iep.rx.lang.scala.Observer]] and
   *         immediately invokes the [[iep.rx.lang.scala.Observer]]r's
   *        [[iep.rx.lang.scala.Observer#onCompleted onCompleted]] method with the
   *         specified scheduler
  ...
  def empty: Observable[Nothing] = {
    toScalaObservable(iep.rx.Observable.empty[Nothing]())
  }
   ...
   *
   * Implementation note: the entire array will be immediately emitted each time an [[iep.rx.lang.scala.Observer]] subscribes.
   * Since this occurs before the [[iep.rx.lang.scala.Subscription]] is returned,
  ...
  def just[T](items: T*): Observable[T] = {
    toScalaObservable[T](iep.rx.Observable.from(items.toIterable.asJava))
  }
 ...
 /** Returns an Observable emitting the value produced by the Future as its single item.
  ...
  def from[T](iterable: Iterable[T]): Observable[T] = {
    toScalaObservable(iep.rx.Observable.from(iterable.asJava))
  }
   ...
   * time as an Observer subscribes to the Observable. This allows an [[iep.rx.lang.scala.Observer]] to easily
   * obtain updates or a refreshed version of the sequence.
   *
   * @param observable
   *            the Observable factory function to invoke for each [[iep.rx.lang.scala.Observer]] that
   *            subscribes to the resulting Observable
   * @tparam T
   *            the type of the items emitted by the Observable
   * @return an Observable whose [[iep.rx.lang.scala.Observer]]s trigger an invocation of the given Observable
   *         factory function
   */
  def defer[T](observable: => Observable[T]): Observable[T] = {
    toScalaObservable[T](iep.rx.Observable.defer[T](() => observable.asJavaObservable.asInstanceOf[iep.rx.Observable[T]]))
  }

  /**
   * Returns an Observable that never sends any items or notifications to an [[iep.rx.lang.scala.Observer]].
   *
   ...
   *
   * @return an Observable that never sends any items or notifications to an [[iep.rx.lang.scala.Observer]]
   */
  def never: Observable[Nothing] = {
    toScalaObservable[Nothing](iep.rx.Observable.never())
  }
  ...
  def zip[A, B, C](obA: Observable[A], obB: Observable[B], obC: Observable[C]): Observable[(A, B, C)] = {
    toScalaObservable[(A, B, C)](iep.rx.Observable.zip[A, B, C, (A, B, C)](obA.asJavaObservable, obB.asJavaObservable, obC.asJavaObservable, (a: A, b: B, c: C) => (a, b, c)))
  }
  ...
  def zip[A, B, C, D](obA: Observable[A], obB: Observable[B], obC: Observable[C], obD: Observable[D]): Observable[(A, B, C, D)] = {
    toScalaObservable[(A, B, C, D)](iep.rx.Observable.zip[A, B, C, D, (A, B, C, D)](obA.asJavaObservable, obB.asJavaObservable, obC.asJavaObservable, obD.asJavaObservable, (a: A, b: B, c: C, d: D) => (a, b, c, d)))
  }
  ...
  def zip[T](observables: Observable[Observable[T]]): Observable[Seq[T]] = {
    ...
    val o = iep.rx.Observable.zip(list, f)
  ...
  def interval(duration: Duration): Observable[Long] = {
    toScalaObservable[java.lang.Long](iep.rx.Observable.interval(duration.length, duration.unit)).map(_.longValue())
  }
  ...
  def interval(period: Duration, scheduler: Scheduler): Observable[Long] = {
    toScalaObservable[java.lang.Long](iep.rx.Observable.interval(period.length, period.unit, scheduler)).map(_.longValue())
  }
  ...
  def timer(initialDelay: Duration, period: Duration): Observable[Long] = {
    toScalaObservable[java.lang.Long](iep.rx.Observable.timer(initialDelay.toNanos, period.toNanos, duration.NANOSECONDS)).map(_.longValue())
  }
  ...
  def timer(initialDelay: Duration, period: Duration, scheduler: Scheduler): Observable[Long] = {
    toScalaObservable[java.lang.Long](iep.rx.Observable.timer(initialDelay.toNanos, period.toNanos, duration.NANOSECONDS, scheduler)).map(_.longValue())
  }
  ...
  def timer(delay: Duration): Observable[Long] = {
    toScalaObservable[java.lang.Long](iep.rx.Observable.timer(delay.length, delay.unit)).map(_.longValue())
  }
  ...
  def timer(delay: Duration, scheduler: Scheduler): Observable[Long] = {
    toScalaObservable[java.lang.Long](iep.rx.Observable.timer(delay.length, delay.unit, scheduler)).map(_.longValue())
  }
  ...
  def using[T, Resource](resourceFactory: => Resource)(observableFactory: Resource => Observable[T], dispose: Resource => Unit): Observable[T] = {
    val jResourceFactory = new iep.rx.functions.Func0[Resource] {
      override def call: Resource = resourceFactory
    }
    val jObservableFactory = new iep.rx.functions.Func1[Resource, iep.rx.Observable[_ <: T]] {
      override def call(r: Resource) = observableFactory(r).asJavaObservable
    }
    toScalaObservable[T](iep.rx.Observable.using[T, Resource](jResourceFactory, jObservableFactory, dispose))
  }
  ...
  def amb[T](sources: Observable[T]*): Observable[T] = {
    toScalaObservable[T](iep.rx.Observable.amb[T](sources.map(_.asJavaObservable).asJava))
  }
  ...
  def combineLatest[T, R](sources: Seq[Observable[T]])(combineFunction: Seq[T] => R): Observable[R] = {
    val jSources = new java.util.ArrayList[iep.rx.Observable[_ <: T]](sources.map(_.asJavaObservable).asJava)
    val jCombineFunction = new iep.rx.functions.FuncN[R] {
      override def call(args: java.lang.Object*): R = combineFunction(args.map(_.asInstanceOf[T]))
    }
    toScalaObservable[R](iep.rx.Observable.combineLatest[T, R](jSources, jCombineFunction))
  }
}
