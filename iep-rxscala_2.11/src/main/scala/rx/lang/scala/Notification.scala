...
/**
 * Emitted by Observables returned by [[iep.rx.lang.scala.Observable.materialize]].
 */
sealed trait Notification[+T] {
  private [scala] val asJavaNotification: iep.rx.Notification[_ <: T]
   ...
   *
   * @param onNext
   *               The function to invoke for an [[iep.rx.lang.scala.Notification.OnNext]] notification.
   * @param onError
   *               The function to invoke for an [[iep.rx.lang.scala.Notification.OnError]] notification.
   * @param onCompleted
   *               The function to invoke for an [[iep.rx.lang.scala.Notification.OnCompleted]] notification.
   */
...
object Notification {
  ...
  private [scala] def apply[T](n: iep.rx.Notification[_ <: T]): Notification[T] = n.getKind match {
    case iep.rx.Notification.Kind.OnNext => new OnNext(n)
    case iep.rx.Notification.Kind.OnCompleted => OnCompleted
    case iep.rx.Notification.Kind.OnError => new OnError(n)
  }
  
  // OnNext, OnError, OnCompleted are not case classes because we don't want pattern matching
  // to extract the iep.rx.Notification
  ...
  object OnNext {
    ...
    def apply[T](value: T): Notification[T] = {
      Notification(iep.rx.Notification.createOnNext[T](value))
    }

    /**
     * Extractor for onNext notifications.
     * @param notification
     *                     The [[iep.rx.lang.scala.Notification]] to be destructed.
     * @return
  ...
  class OnNext[+T] private[scala] (val asJavaNotification: iep.rx.Notification[_ <: T]) extends Notification[T] {
  ...
  object OnError {
    ...
    def apply[T](error: Throwable): Notification[T] = {
      Notification(iep.rx.Notification.createOnError[T](error))
    }
     ...
     *
     * @param notification
     *                     The [[iep.rx.lang.scala.Notification]] to be deconstructed
     * @return
  ...
  class OnError[+T] private[scala] (val asJavaNotification: iep.rx.Notification[_ <: T]) extends Notification[T] {
  ...
  object OnCompleted extends Notification[Nothing] {
    ...
    val asJavaNotification = iep.rx.Notification.createOnCompleted[Nothing]()
  }
