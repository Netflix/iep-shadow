...
import iep.rx.lang.scala.schedulers._
...
trait Scheduler {
  ...
  private [scala] val asJavaScheduler: iep.rx.Scheduler
  ...
  /**
   * Retrieve or create a new [[iep.rx.lang.scala.Worker]] that represents serial execution of actions.
   * <p>
   * When work is completed it should be unsubscribed using [[iep.rx.lang.scala.Worker unsubscribe]].
   * <p>
   * Work on a [[iep.rx.lang.scala.Worker]] is guaranteed to be sequential.
   *
...
object Worker {
  def apply(worker: iep.rx.Scheduler.Worker): Worker = new Worker {
...
trait Worker extends Subscription {
  private [scala] val asJavaWorker: iep.rx.Scheduler.Worker
...
private [scala] object Scheduler {
  def apply(scheduler: iep.rx.Scheduler): Scheduler = scheduler match {
    case s: iep.rx.schedulers.TestScheduler => new TestScheduler(s)
    case s: iep.rx.Scheduler => new Scheduler{ val asJavaScheduler = s }
  }
