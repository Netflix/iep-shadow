 ...
 * the `iep.rx.Observable` directly instead of using `iep.rx.lang.scala.Observable` or if one wants
 ...
 * use [[iep.rx.lang.scala.JavaConversions]].
 */
object ImplicitFunctionConversions {

//  implicit def schedulerActionToFunc2[T](action: (Scheduler, T) => Subscription): Func2[iep.rx.Scheduler, T, iep.rx.Subscription] with Object {def call(s: iep.rx.Scheduler, t: T): iep.rx.Subscription} =
//    new Func2[iep.rx.Scheduler, T, iep.rx.Subscription] {
//      def call(s: iep.rx.Scheduler, t: T): iep.rx.Subscription = {
//        action(iep.rx.lang.scala.Scheduler(s), t).asJavaSubscription
//      }
//    }
  ...
  implicit def scalaAction1ToOnSubscribe[T](f: Subscriber[T] => Unit) =
    new iep.rx.Observable.OnSubscribe[T] {
      def call(s: iep.rx.Subscriber[_ >: T]): Unit = {
        f(s)
      }
    }
