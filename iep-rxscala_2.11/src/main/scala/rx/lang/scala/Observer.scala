...
*
* After an Observer calls an [[iep.rx.lang.scala.Observable]]'s `subscribe` method, the Observable
...
trait Observer[-T] {
  ...
  private [scala] val asJavaObserver: iep.rx.Observer[_ >: T] = new iep.rx.Observer[T] {
 ...
 *
 * The [[iep.rx.lang.scala.Observable]] calls this closure 0 or more times.
 *
 * The [[iep.rx.lang.scala.Observable]] will not call this method again after it calls either `onCompleted` or `onError`.
 */
  def onNext(value: T): Unit = {}

  /**
  * Notifies the Observer that the [[iep.rx.lang.scala.Observable]] has experienced an error condition.
  *
  * If the [[iep.rx.lang.scala.Observable]] calls this method, it will not thereafter call `onNext` or `onCompleted`.
  */
  def onError(error: Throwable): Unit= {}

  /**
   * Notifies the Observer that the [[iep.rx.lang.scala.Observable]] has finished sending push-based notifications.
   *
   * The [[iep.rx.lang.scala.Observable]] will not call this method if it calls `onError`.
   */
...
object Observer extends ObserverFactoryMethods[Observer] {
   ...
   */
  private [scala] def apply[T](observer: iep.rx.Observer[T]) : Observer[T] = {
     new Observer[T] {
  ...
  def apply[T](onNext: T => Unit, onError: Throwable => Unit, onCompleted: () => Unit): Observer[T] = {
    ...
    // Java calls XXX; Scala receives XXX.
    Observer(new iep.rx.Observer[T] {
