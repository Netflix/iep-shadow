...
import iep.rx.lang.scala.ImplicitFunctionConversions._
import iep.rx.lang.scala.Observable
import iep.rx.observables.{BlockingObservable => JBlockingObservable}


/**
 * An Observable that provides blocking operators.
 * 
 * You can obtain a BlockingObservable from an Observable using [[iep.rx.lang.scala.Observable.toBlocking]]
 */
  ...
  extends AnyVal
{
  ...
  def mostRecent[U >: T](initialValue: U): Iterable[U] = {
    val asJavaU = asJava.asInstanceOf[iep.rx.observables.BlockingObservable[U]]
...
private[observables] class WithFilter[+T] (p: T => Boolean, asJava: iep.rx.observables.BlockingObservable[_ <: T]) {
  import iep.rx.lang.scala.ImplicitFunctionConversions._
