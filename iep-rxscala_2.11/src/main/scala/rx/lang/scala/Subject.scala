...
import iep.rx.lang.scala.subjects.SerializedSubject

/**
* A Subject is an Observable and an Observer at the same time.
*/
trait Subject[T] extends Observable[T] with Observer[T] {
  private [scala] val asJavaSubject: iep.rx.subjects.Subject[_ >: T, _<: T]

  val asJavaObservable: iep.rx.Observable[_ <: T] = asJavaSubject

  override val asJavaObserver: iep.rx.Observer[_ >: T] = asJavaSubject
   ...
   *
   * To protect a [[Subject]] from this danger, you can convert it into a [[iep.rx.lang.scala.subjects.SerializedSubject SerializedSubject]]
   * with code like the following:
   * {{{
   * mySafeSubject = myUnsafeSubject.toSerialized
   * }}}
   *
   * @return [[iep.rx.lang.scala.subjects.SerializedSubject SerializedSubject]] wrapping the current [[Subject]]
   */
...
object Subject {
   ...
   */
  def apply[T](): Subject[T] = new iep.rx.lang.scala.subjects.PublishSubject[T](iep.rx.subjects.PublishSubject.create())
}
