 ...
 * 
 * A well-behaved Subject does not interleave its invocations of the [[iep.rx.lang.scala.Subject.onNext onNext]],
 * [[iep.rx.lang.scala.Subject.onCompleted onCompleted]], and [[iep.rx.lang.scala.Subject.onError onError]] methods of
 * its [[iep.rx.lang.scala.Subject]]s; it invokes `onCompleted` or `onError` only once; and it never invokes `onNext`
...
class SerializedSubject[T] private [scala] (val asJavaSubject: iep.rx.subjects.SerializedSubject[_ >: T, _ <: T]) extends Subject[T]
