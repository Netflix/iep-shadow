...
object TestSubject {
  ...
  def apply[T](scheduler: TestScheduler): TestSubject[T] = {
    new TestSubject[T](iep.rx.subjects.TestSubject.create(scheduler.asJavaScheduler))
  }
}

class TestSubject[T] private[scala] (val asJavaSubject: iep.rx.subjects.TestSubject[T]) extends Subject[T]  {}
