...
import iep.rx.lang.scala.{Scheduler, Subject}
...
object ReplaySubject {
  ...
  def apply[T](): ReplaySubject[T] = {
    new ReplaySubject[T](iep.rx.subjects.ReplaySubject.create())
  }
  ...
  def apply[T](capacity: Int): ReplaySubject[T] = {
    new ReplaySubject[T](iep.rx.subjects.ReplaySubject.create(capacity))
  }
  ...
  def withSize[T](size: Int): ReplaySubject[T] = {
    new ReplaySubject[T](iep.rx.subjects.ReplaySubject.createWithSize(size))
  }
  ...
  def withTime[T](time: Duration, scheduler: Scheduler): ReplaySubject[T] = {
    new ReplaySubject[T](iep.rx.subjects.ReplaySubject.createWithTime(time.length, time.unit, scheduler.asJavaScheduler))
  }
  ...
  def withTimeAndSize[T](time: Duration, size: Int, scheduler: Scheduler): ReplaySubject[T] = {
    new ReplaySubject[T](iep.rx.subjects.ReplaySubject.createWithTimeAndSize(time.length, time.unit, size, scheduler.asJavaScheduler))
  }
}

class ReplaySubject[T] private[scala] (val asJavaSubject: iep.rx.subjects.ReplaySubject[T]) extends Subject[T]  {}
